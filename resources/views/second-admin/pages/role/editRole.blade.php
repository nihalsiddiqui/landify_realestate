                        ...
                        <div class="col-md-12 col-12">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    @endif
                                    <form class="form form-horizontal" role="form" method="post" action="{{ route('admin.role.update', ['id' => $role->id]) }}">
                                        {{csrf_field()}}
                                                    ...
                                                    <div class="col-sm-9">
                                                        <select class="form-control select2" multiple="multiple"
                                                            ...
                                                            @foreach($permissions as $permission)
                                                                <option value="{{$permission->id}}" {{ in_array($permission->id, $role->permissions()->get()->pluck('id')->toArray()) ? 'selected' : ''}}>{{$permission->name}}</option>
                                                            @endforeach
                                                        </select>
                                                        @error("permissions")
                                                        <span class="text-danger">{{ $message }}</span>
